...
from spaceone.cost_analysis.connector import AWSS3Connector
...
class JobManager(BaseManager):
    ...
    def get_tasks(
        self, options, secret_data, schema, start, last_synchronized_at, domain_id
    ):
        tasks = []
        changed = []
        ...
        if total_count > 0:
            ...
            for service_account_info in response.get("results", []):
                ...
                _LOGGER.debug(
                    f"[get_tasks] service_account({service_account_id}): name={service_account_name}, account_id={account_id}"
                )
                task_options = {
                    "is_sync": is_sync,
                ...
                if is_sync == "false":
                    ...
                    changed.append(
                        {"start": first_sync_time, "filter": {"account": account_id}}
                    )
                else:
